...
class core_renderer extends \core_renderer {
	public function getNewCourse()
	{
		global $cfg;
		$url="";
		$context = get_category_or_system_context($CFG->defaultrequestcategoryd);
 ...
 public function face($user = null, $withlinks = null) {
        global $USER, $CFG;
        ...
        $opts = user_get_user_navigation_info($user, $this->page);
       	$userdetals=user_get_user_details($user,null);
        $avatarclasses = "avatar";
        $avatarcontents = $userdetals[ 'profileimageurl'];
...
return $avatarcontents ; 
       
    }
    public function face1 ($user = null, $withlinks = null) {
    	global $USER, $CFG;
    	require_once($CFG->dirroot . '/user/lib.php');
    	...
    	if (!empty($opts->metadata['asotheruser'])) {
    	  ...
    	  $html = html_writer::start_div('nav-1');
    	  $html .=html_writer::start_tag('img', array('src' => $avatarcontents, 'class' => 'img-circle'));
    	  $html .= html_writer::end_tag('img');
    	  $html .=html_writer::start_tag('span', array( 'id'=>'name','class' => ' list-group-item'));
    	  
    	  $html .=$opts->metadata['userfullname'];
    ...
    public function full_header() {
     ...
     //   $html .= $this->context_header();//coton
    //-------------------------------------------------    
...
//             $html .= $this->single_button($url, get_string('addnewcourse'), 'get');
//         }

//     if ($PAGE->user_allowed_editing()) {
//         $buttons = $html->edit_button($PAGE->url);
