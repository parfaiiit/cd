...
class theme_essential_core_course_renderer extends core_course_renderer {
	...
	//	$content = html_writer::start_tag('div', $attributes);
		$content = html_writer::start_tag('div',  array('class' => 'courses frontpage-course-list-all container-fluid'));
		if (!empty($pagingbar)) {
			$content .= $pagingbar;
		}
	
		$coursecount = 1;
		$content .= html_writer::start_tag('div', array('class' => 'row-fluit  p-y-1'));
		foreach ($courses as $course) {
			$content .= $this->coursecat_coursebox($chelper, $course, 'span3');
	...
	protected function coursecat_category(coursecat_helper $chelper, $coursecat, $depth) {
		...
		$content .= html_writer::tag ( 'span', $coursescount, array (
				'title' => $coursecounttitle,
		...
		if (! empty ( $image )) {
			$categoryrepresentation = html_writer::start_tag ( 'div', array (
					'class' => 'categoryimage' 
			) );
			$categoryrepresentation .= html_writer::empty_tag ( 'img', array (
					'src' => $image,
					'class' => 'img-responsive' 
			) );
			$categoryrepresentation .= html_writer::end_tag ( 'div' );
		} else if (! empty ( $icon )) {
			$categoryrepresentation = html_writer::tag ( 'span', '', array (
					'aria-hidden' => 'true',
					'class' => 'fa fa-' . $icon 
			) );
		} else {
	...
	protected function coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '') {
		...
		$classes = trim('card coursebox content clearfix');
	...
	protected function coursecat_coursebox_content(coursecat_helper $chelper, $course) {
        ...
        $content .= html_writer::start_tag('div', array('class' => 'img-reSize ',

                                                    ));
    	$content .= $this->get_course_summary_image($course);
        $content .= html_writer::end_tag('div');
    
    	$content .= html_writer::start_tag('div');//$content .= html_writer::start_tag('div', array('class' => 'card-block '));
    	$content .= "<h4 class='card-title text-center'>". $coursenamelink ."</h4>";
...
//     	}
	...
	protected function coursecat_coursebox_content1(coursecat_helper $chelper, $course) {
		if (! $this->enablecategoryicon) {
		...
		if ($coursehassummary) {
			...
			$content .= $chelper->get_course_formatted_summary ( $course, array (
					'overflowdiv' => true,
					'noclean' => true,
					'para' => false 
			) );
		...
		if ($coursehascontacts) {
			...
			foreach ( $course->get_course_contacts () as $userid => $coursecontact ) {
				...
				$name = $faiconsettinghtml . $coursecontact ['rolename'] . ': ' . html_writer::link ( new moodle_url ( '/user/view.php', array (
						'id' => $userid,
						'course' => SITEID 
		...
		foreach ( $courses as $course ) {
			...
			foreach ( $modinfo->get_section_info_all () as $section => $thissection ) {
				...
				if (( string ) $thissection->name !== '') {
					$sectionname = format_string ( $thissection->name, true, array (
	...
	protected function get_course_summary_image($course) {
		...
		if (empty($contentimage)) {
			$url = $CFG->wwwroot . "/theme/essential/pix/default_course.jpg";
