...
class block_homework_mark_page extends e\block_homework_form_page_base {
    ...
    protected function get_form_settings() {
        ...
        $users = block_homework_moodle_utils::get_assignment_participants_and_statuses($this->cmid); //获取用户信息
        $learners = array();
        ...
        foreach ($users as $user) {
            ...
            $table->add_row();
            //用户名列
            $userlink = new e\htmlHyperlink(null, $user->lastname . ' ' . $user->firstname, $CFG->wwwroot . '/user/view.php?id=' .
            ...
            if (($this->assignment->textsubmissionenabled) || ($this->assignment->filesubmissionenabled)) {
            	if(  ($this->assignment->textsubmissionenabled)&& isset($user->submissionid)){
            		$assign = new assign(context_module::instance($this->assignment->id), null, null);
            		$PAGE->get_renderer('mod_assign');
            		//放大在线提交内容
            		$viewstr = get_string('view' . substr(assignsubmission, strlen('assign')), 'assign');
            		$icon = ($PAGE->get_renderer('mod_assign'))->pix_icon('t/preview', $viewstr);
            		$urlparams = array('id' => $this->assignment->id,
            				'sid'=>$user->submissionid,
            				'gid'=>$user->submissionid,
            				'plugin'=>'onlinetext',
            				'action'=>'viewplugin' .'assignsubmission',
            				'returnaction'=>'grading',
            				'returnparams'=>http_build_query($returnparams));
            		$url = new moodle_url('/mod/assign/view.php', $urlparams);
            		$submissionstuff =$PAGE->get_renderer('mod_assign')->action_link($url, $icon);
            		//放大在线提交内容
            	 $submissionstuff .=$assign->render_editor_content('submissions_onlinetext',
                                                             $user->submissionid,
                                                             'onlinetext',
                                                             'onlinetext',
                ...
                if ($user->submissionnumfiles > 0) { //作业附加文件
            ...
            $lname = new e\htmlHiddenInput('name_' . $user->userid, 'name_' . $user->userid,   $user->lastname. ' ' .
                  $user->firstname );
